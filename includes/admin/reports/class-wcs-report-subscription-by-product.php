		...
		switch ( $column_name ) {
			...
			case 'product_name':
				...
				if ( $report_item->parent_product_id > 0 ) {
					edit_post_link( esc_html( $report_item->product_name ), ' - ', null, $report_item->parent_product_id );
				} else {
					edit_post_link( esc_html( $report_item->product_name ), null, null, $report_item->product_id );
				}
				break;
