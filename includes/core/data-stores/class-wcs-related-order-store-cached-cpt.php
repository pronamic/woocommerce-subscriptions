...
class WCS_Related_Order_Store_Cached_CPT extends WCS_Related_Order_Store_CPT implements WCS_Cache_Updater {
	...
	protected function update_related_order_id_cache( $subscription, array $related_order_ids, $relation_type ) {
		...
		/*
		 * Memory cache busting needs to happen before [add|update]_meta
		 * because those functions end up triggering the `woocommerce_update_order` hook
		 * and the meta cache needs to be up-to-date by then.
		 */
		$this->bust_batch_processing_mode_memory_cache( $subscription );
	...
	public function get_related_order_ids_by_types( WC_Order $subscription, $related_order_types ) {
		...
		$this->start_batch_processing_mode( $subscription );
		...
		$this->stop_batch_processing_mode( $subscription );
	 ...
	 *
	 * @param string $subscription The subscriptions being cached.
	 */
	private function stop_batch_processing_mode( $subscription ) {
		$cache_key = $this->get_batch_processing_cache_key( $subscription );
	...
	/**
	 * Busts the batch processing mode memory cache for a subscription without stopping the batch processing mode.
	 *
	 * @param WC_Subscription $subscription the subscription being cached.
	 */
	private function bust_batch_processing_mode_memory_cache( $subscription ) {
		$cache_key = $this->get_batch_processing_cache_key( $subscription );
		unset( self::$subscription_meta_cache[ $cache_key ] );
	}
